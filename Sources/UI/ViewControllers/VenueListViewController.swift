...
class VenueListViewController: UITableViewController, UITextFieldDelegate {
    ...
    func search(what: String, near: String){
        ...
        searchForVenues(what, near: near).then { venues -> () in
            let venueDetailsPromises = venues.map {
                self.getVenueDetails($0.id)
            }
            when(venueDetailsPromises).then { venuesWithDetails  -> () in
                self.venues = venuesWithDetails
